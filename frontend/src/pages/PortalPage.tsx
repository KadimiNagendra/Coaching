...
export default function PortalPage() {
  ...
  const { data: results = [] } = useQuery({ queryKey: ['portal-results'], queryFn: api.portalResults, enabled: isParent });
  ...
  return (
    <Box>
      ...
      {isParent && (
        <Box sx={{ mb: 2 }}>
          <Typography variant="h6" sx={{ mb: 1.5, fontWeight: 700 }}>Children Performance</Typography>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2 }}>
            ...
            {performance.length === 0 && (
              <Card>
                <CardContent>
                  <Typography color="text.secondary">No linked students found for performance tracking.</Typography>
                </CardContent>
              </Card>
            )}
          </Box>
